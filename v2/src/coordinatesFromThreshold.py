...
def findLongitudeLatitude2(lat1, long1, distance, bearingPoint1):
    ...
    latitude2 = math.asin(math.sin(lat) * math.cos(angularDistance) +
                          math.cos(lat) * math.sin(angularDistance) * math.cos(bearingPoint))
    longitude2 = long + math.atan2(math.sin(bearingPoint) * math.sin(angularDistance) * math.cos(
        (lat)), math.cos(angularDistance) - math.sin((lat)) * math.sin((latitude2)))
