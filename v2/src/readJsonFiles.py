...
def getRunwayJsonFiles(airportDetails, airportFiles):
    ...
    if len(airportDetails['runwaysTablePageNumber']) == 1:
        runwayFileName = fs.removeFileExtension(airportFiles['runways'],'.pdf')
        runwayFileName = runwayFileName+airportDetails['runwaysTablePageNumber'][0]
        ...
        for n in range(len(airportDetails['runwaysTablePageNumber'])):
            ...
            runwaysFileNames.append(fileName+airportDetails['runwaysTablePageNumber'][n])
...
def getTerminalStandsFiles(airportDetails, airportFiles):
    ...
    if (len(airportDetails['terminalStandsTablePageNumber']) == 1):
        terminalStandsFileName = fs.removeFileExtension(airportFiles['terminalStands'],'.pdf')
        terminalStandsFileName = terminalStandsFileName+airportDetails['terminalStandsTablePageNumber'][0]
        ...
        for n in range (len(airportDetails['terminalStandsTablePageNumber'])):
            terminalStandsFileName = fs.removeFileExtension(airportFiles['terminalStands'],'.pdf')
            terminalStandsFilesNames.append(terminalStandsFileName+airportDetails['terminalStandsTablePageNumber'][n])
